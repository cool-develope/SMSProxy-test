...
func sendSmsHandler(smsProxy smsproxy.SmsProxy) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		var r SendSmsRequest
		err := decoder.Decode(&r)
		if err != nil {
			handleError(writer, 400, err)
			return
		}
		result, err := smsProxy.Send(smsproxy.SendMessage{
			PhoneNumber: r.PhoneNumber,
			Message:     r.Content,
		})
		if _, ok := err.(*smsproxy.ValidationError); ok {
			handleError(writer, 400, err)
			return
		} else if err != nil {
			handleError(writer, 500, err)
			return
		}
